...
BEGIN
    ...
    ('session_timeout_minutes', '30', 'number', 'User session timeout'),
...
-- ====================================================================
-- ====================================================================
-- PART 2: ADDITIONAL TABLES FOR AI + MFA FEATURES
-- (Merged from db_additions.sql — run automatically after Part 1)
-- ====================================================================
-- ====================================================================
...
-- Using a safe procedure to avoid errors if column already exists
DROP PROCEDURE IF EXISTS sp_add_entry_hash_column;
DELIMITER $$
CREATE PROCEDURE sp_add_entry_hash_column()
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE audit_log
            ADD COLUMN entry_hash VARCHAR(64) NULL AFTER created_at;
    END IF;
END$$
DELIMITER ;
CALL sp_add_entry_hash_column();
DROP PROCEDURE IF EXISTS sp_add_entry_hash_column;
...
-- ====================================================================
-- Run these lines to create the app user (change password if you want)
...
-- FINAL VERIFICATION
-- ====================================================================
SELECT 'Smart Care Database Setup Complete!' AS Status;
SELECT CONCAT('Total Tables: ', COUNT(*)) AS TableCount
FROM information_schema.tables
WHERE table_schema = 'hospital_erp';
SELECT CONCAT('Total Patients: ', COUNT(*)) AS PatientCount FROM patients;
SELECT CONCAT('Total Users: ', COUNT(*)) AS UserCount FROM users;
SELECT CONCAT('Total Appointments: ', COUNT(*)) AS ApptCount FROM appointments;
